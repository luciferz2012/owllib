...
class Ontology:
    ...
    def add_relation(self, s, p, o, sync=False):
    ...
    def add_list_item(self, containerClass, first, contentClass, rest):
    ...
    def define_union(self, unionClass, classList):
        first = BNode()
        unionClass.triples.add((unionClass.uri, OWL.unionOf, first))
        for owlClass in classList[:-1]:
            first = self.add_list_item(unionClass, first, owlClass, BNode())
        self.add_list_item(unionClass, first, classList[-1], RDF.nil)
        return unionClass

    def add_sub_relation(self, child, relation, parent, sync=False):
        self.add_relation(child, relation, parent, sync)
        child.parents.add(parent)
        parent.children.add(child)

    def link_sub_class(self, child, parent, sync=False):
        self.add_relation(child, RDFS.subClassOf, parent, sync)

    def link_sub_property(self, child, parent, sync=False):
        self.add_relation(child, RDFS.subProperty, parent, sync)
    ...
    def get_definitions(self, entity):
        ...
        definitions = [obj for obj in
                       self.graph.objects(entity.uri, URIRef("http://purl.obolibrary.org/obo/IAO_0000115"))]
    ...
    def get_triples(self, entity):
        ...
        return set(self.graph.triples((entity.uri, None, None))) | set(
            self.graph.triples((None, entity.uri, None))) | set(self.graph.triples((None, None, entity.uri)))
